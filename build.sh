...
# Deploy to EC2
ssh -i "$HOME/Downloads/"stock-keypair.pem  ubuntu@ec2-3-17-133-210.us-east-2.compute.amazonaws.com "sed -i 's|image: psainghak/stock-management:staging-v[0-9]*|image: kimsourtann/stock-management:staging-v"$new_version"|' stockmanagement/docker-compose.yml &&   docker compose -f stockmanagement/docker-compose.yml up -d"