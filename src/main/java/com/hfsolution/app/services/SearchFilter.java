...
public class SearchFilter<T> {
                ...
                for (SearchRequest searchRequest : searchRequests) {
                    ...
                    switch (searchRequest.getOperator()) {
                        ...
                        case GREATER_THAN:
                            ...
                            Predicate greaterThan = cb.greaterThan(root.get(searchRequest.getColumn()), (String)searchRequest.getValue());
                            predicates.add(greaterThan);
                            break;
                        case LESS_THAN:
                            Predicate lessThan = cb.lessThan(root.get(searchRequest.getColumn()), (String)searchRequest.getValue());
                            predicates.add(lessThan);
                            break;
                        case IN:
                            String[] split = ((String)searchRequest.getValue()).split(",");
                            Predicate in = root.get(searchRequest.getColumn()).in(Arrays.asList(split));
                            predicates.add(in);
                            break;
                        case BETWEEN:
                            String[] split2 = ((String)searchRequest.getValue()).split(",");
    ...
    // public Specification<T> getSearchSpecification(List<SearchRequest> searchRequests, GlobalOperator globalOperator,  Class<Enum> clazz){
    //     return new Specification<T>() {
    //         @Override
    //         public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb){
    //             if (searchRequests == null || searchRequests.isEmpty()) {
    ...
    //                 switch (searchRequest.getOperator()) {
    //                     case EQUAL:
    //                          Predicate equal = cb.equal(root.get(searchRequest.getColumn()), EnumConvertor.fromString(clazz, searchRequest.getValue()));
    //                         predicates.add(equal);
    //                         break;
    //                     case LIKE:
    //                         Predicate like = cb.like(root.get(searchRequest.getColumn()), "%"+searchRequest.getValue()+"%");
